module bit_population_counter_tb #(
  parameter WIDTH            = 64,
  //width of array of random generated data
  //data generated by 32 bit blocks (+1 block for width < 32 without cell)
  parameter MAX_WIDTH_RANDOM = WIDTH/32 + 1
);
  ...
  task add_tasks( mailbox #( task_struct ) generated_tasks,
    ...
    while(number_of_tasks-- > 0)
      begin
        logic [MAX_WIDTH_RANDOM-1:0][31:0] tmp_data;     
        task_struct new_task;
        new_task.skip = skip;
        new_task.cnt  = '0;
        
        for (int i = 0; i < MAX_WIDTH_RANDOM; i++)
          tmp_data[i] = $urandom_range(2**32-1, 0);

        new_task.data = tmp_data;
        new_task.cnt  = $countones(new_task.data);
      ...
      begin
        ...
        if( data_val_o === 1'b1 )
          begin
            ...
            if( tmp_cnt !== data_o)
              begin
                $display("Wrong data in time: ", $time, "  WIDTH = ", WIDTH, "   expected: ", tmp_cnt, "  data_out: ", data_o);
                status_accuracy = 0;
                $stop();
              end
          end
